"""
chunk_and_embed.py  —  Christianity / Bible (KJV)

Key improvement over v1:
  • Topic preambles injected into each chunk so FAISS retrieval matches
    natural-language questions like "Sermon on the Mount", "Lord's Prayer",
    "Parable of the Prodigal Son", etc. — even though those exact labels
    never appear in the raw verse text.
  • Chapter-range tagging maps every chapter of every book to a known
    topic/passage label.
  • Synonym expansion adds alternate phrasings so queries like
    "how to pray", "what is salvation", "turn the other cheek" all
    retrieve the right chunks.
  • Chunk size reduced to 400 words (overlap 80) for tighter retrieval
    precision — Bible verses are short, so large chunks dilute relevance.
"""
...
CHUNK_SIZE    = 400   # tighter chunks → better retrieval precision for short verses
CHUNK_OVERLAP = 80
...
# TOPIC PREAMBLE MAP
#
# Format:  (book, chapter_start, chapter_end) -> preamble string
#
# The preamble is prepended to every chunk that falls within
# that chapter range BEFORE embedding.  This means a user asking
# "Sermon on the Mount" will match the Matthew 5-7 chunks even
# though the phrase never appears in the KJV text itself.
#
# Preambles include:
#   • The canonical name of the passage
#   • Common synonyms / alternate phrasings
#   • Key themes so topical questions also match
# ────────────────────────────────────────────────────────────────
...
def _get_topic_preamble(book: str, chapter: int) -> str:
    """Return the topic preamble string for a given book + chapter, or ''."""
...
#
# For chunks that contain ANY of the trigger phrases (checked against
# the raw text), prepend extra synonym terms so alternate user phrasings
# also retrieve the right chunks.
# ────────────────────────────────────────────────────────────────
...
def _get_synonym_expansion(text: str) -> str:
    """Return any synonym expansions triggered by content in the chunk text."""
...
# ────────────────────────────────────────────────────────────────
# Chunking  —  with topic preamble + synonym injection
# ────────────────────────────────────────────────────────────────

print("Chunking corpus with topic preambles and metadata...")
all_chunks = []
...
for item in tqdm(corpus, desc="Chunking"):
    ...
    # Derive testament from book name if not supplied
    ...
    for raw_chunk in chunk_text(raw_text):
        ...
        # 1. Topic preamble (passage label + synonyms) — most important
        topic = _get_topic_preamble(section, chapter)
        if topic:
            parts.append(topic)

        # 2. Book + testament context header
        parts.append(f"Book: {section}. Testament: {testament}. Genre: {genre}.")

        # 3. The actual scripture text
        parts.append(raw_chunk)

        # 4. Synonym expansions triggered by content
        ...
        all_chunks.append({
            "text":       raw_chunk,       # stored as-is — no preamble in DB display
            "embed_text": embed_text,      # used for embedding only
            "source":     section,
            "book":       section,
            "testament":  testament,
            "genre":      genre,
            ...
            "source_url": (
                ...
                f"?search={section.replace(' ', '+')}&version=KJV"
            ),
        })
...
# ────────────────────────────────────────────────────────────────
# Embeddings  —  encode embed_text (with preambles), store raw text
# ────────────────────────────────────────────────────────────────

print("\nLoading embedding model...")
model = SentenceTransformer(MODEL_NAME)

print("Creating embeddings (using enriched embed_text with topic preambles)...")
...
print("\n" + "─" * 60)
print("Done. Files to upload to HuggingFace:")
for path, note in [
    ...
    print(f"  {path.name:<25} {mb:>7.1f} MB   {note}")
print("─" * 60)
print(f"\nTotal chunks embedded with topic preambles: {len(all_chunks):,}")
print("Key improvement: FAISS now embeds enriched text (preamble + scripture + synonyms)")
print("but stores only the clean scripture text in the DB for display.")